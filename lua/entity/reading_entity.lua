-- KanjiData SDK Reading entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReadingEntity = {}
ReadingEntity.__index = ReadingEntity


function ReadingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReadingEntity)
  self._name = "reading"
  self._client = client
...
function ReadingEntity:get_name()
  return self._name
end


function ReadingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReadingEntity.new(self._client, opts)
end
...
function ReadingEntity:mark_deleted()
  self._deleted = true
end


function ReadingEntity:deleted()
  return true == self._deleted
end


function ReadingEntity:data_set(args)
  if args ~= nil then
...
function ReadingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReadingEntity:match_set(args)
  if args ~= nil then
...
function ReadingEntity:match_get()
...
function ReadingEntity:stream(action, args, callopts)
...
---@param reqmatch ReadingLoadMatch
---@param ctrl? table
---@return Reading
---@return string? err
function ReadingEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReadingEntity:_run_op(ctx, post_done)
  local utility = self._utility
