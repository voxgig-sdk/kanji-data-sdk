...
local KanjiEntity = {}
KanjiEntity.__index = KanjiEntity


function KanjiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, KanjiEntity)
  self._name = "kanji"
  self._client = client
...
function KanjiEntity:get_name()
  return self._name
end


function KanjiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return KanjiEntity.new(self._client, opts)
end
...
function KanjiEntity:mark_deleted()
  self._deleted = true
end


function KanjiEntity:deleted()
  return true == self._deleted
end


function KanjiEntity:data_set(args)
  if args ~= nil then
...
function KanjiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function KanjiEntity:match_set(args)
  if args ~= nil then
...
function KanjiEntity:match_get()
...
function KanjiEntity:stream(action, args, callopts)
...
---@param reqmatch KanjiLoadMatch
---@param ctrl? table
---@return Kanji
---@return string? err
function KanjiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function KanjiEntity:_run_op(ctx, post_done)
  local utility = self._utility
