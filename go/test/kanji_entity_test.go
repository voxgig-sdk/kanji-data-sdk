...
func TestKanjiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		kanjiRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.kanji")))
...
func kanjiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KANJI_DATA_TEST_KANJI_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KANJI_DATA_TEST_KANJI_ENTID"])
