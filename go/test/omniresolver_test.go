...
import (
	...
	omni "github.com/voxgig-sdk/kanji-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KanjiDataError); is {
				out := map[string]any{
					"name":    "KanjiDataError",
...
func omniCtx(arg any, client *sdk.KanjiDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KanjiDataSDK); is {
			client = live
