...
func (sdk *KanjiDataSDK) OptionsMap() map[string]any {
...
func (sdk *KanjiDataSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *KanjiDataSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *KanjiDataSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *KanjiDataSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *KanjiDataSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *KanjiDataSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("KanjiDataSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *KanjiDataSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *KanjiDataSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("KanjiDataSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
