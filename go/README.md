# KanjiData Golang SDK



The Golang SDK for the KanjiData API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Kanji(nil)` — each with the same small set of operations (`Load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/kanji-data-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/kanji-data-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/kanji-data-sdk/go=../kanji-data-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *KanjiDataSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### KanjiDataSDK methods
...
| `Kanji` | `(data map[string]any) KanjiDataEntity` | Create a Kanji entity instance. |
| `Reading` | `(data map[string]any) KanjiDataEntity` | Create a Reading entity instance. |
| `Word` | `(data map[string]any) KanjiDataEntity` | Create a Word entity instance. |

### Entity interface (KanjiDataEntity)

All entities implement the `KanjiDataEntity` interface.
...
Create an instance: `kanji := client.Kanji(nil)`
...
```go
kanji, err := client.Kanji(nil).Load(map[string]any{"id": "kanji_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(kanji) // the loaded record
```


### Reading

Create an instance: `reading := client.Reading(nil)`
...
```go
reading, err := client.Reading(nil).Load(map[string]any{"id": "reading_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(reading) // the loaded record
```
...
```
github.com/voxgig-sdk/kanji-data-sdk/go/
├── kanji-data.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/kanji-data-sdk/go`) re-exports everything needed
