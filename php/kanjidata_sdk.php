...
class KanjiDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = KanjiDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = KanjiDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!KanjiDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, KanjiDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return KanjiDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = KanjiDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = KanjiDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = KanjiDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new KanjiDataError($op . "_allow",
                "KanjiDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = KanjiDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new KanjiDataError("graphql_error",
                "KanjiDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Kanji($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_kanji === null) {
                $this->_kanji = new KanjiEntity($this, null);
            }
            return $this->_kanji;
        }
        return new KanjiEntity($this, $data);
    }
    ...
    public function Reading($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_reading === null) {
                $this->_reading = new ReadingEntity($this, null);
            }
            return $this->_reading;
        }
        return new ReadingEntity($this, $data);
    }
