...
class KanjiDataNetsimFeature extends KanjiDataBaseFeature
{
    ...
    private function _simulate(KanjiDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(KanjiDataContext $ctx, array $applied): void
    {
        $client = $this->client;
