...
class KanjiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new KanjiEntity($this->_client, $opts);
    }

    /**
     * @param Kanji|array $args Kanji data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = KanjiDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Kanji|array The current Kanji data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Kanji fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = KanjiDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Kanji fields).
     */
     ...
     * Load a single Kanji.
     *
     * @param KanjiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed KanjiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Kanji|array The loaded Kanji as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = KanjiDataHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
