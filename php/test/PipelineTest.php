...
{
    private static function utility(): KanjiDataUtility
    {
        return new KanjiDataUtility();
    }
    ...
    private static function ctx(array $over = []): KanjiDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KanjiDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KanjiDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KanjiDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KanjiDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KanjiDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KanjiDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KanjiDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KanjiDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KanjiDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KanjiDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KanjiDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KanjiDataResponse([]),
            'result' => new KanjiDataResult([]),
        ]);
        [, $err] = KanjiDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new KanjiDataSpec([]),
            'response' => null,
            'result' => new KanjiDataResult([]),
        ]);
        [, $err] = KanjiDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KanjiDataSpec(['step' => 's']),
            'response' => new KanjiDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KanjiDataResult([]),
        ]);
        [, $err] = KanjiDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KanjiDataSpec(['step' => 's']),
            'response' => new KanjiDataResponse(self::resp(200, ['v' => 1])),
            'result' => new KanjiDataResult([]),
        ]);
        [, $err] = KanjiDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KanjiDataSpec(['step' => 's']),
            'response' => new KanjiDataResponse(self::resp(200, ['v' => 2])),
            'result' => new KanjiDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KanjiDataMakeResponse::call($ctx);
    ...
    {
        $preset = new KanjiDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KanjiDataSpec([]),
            'response' => new KanjiDataResponse([]),
            'result' => new KanjiDataResult([]),
        ...
        [$got, $err] = KanjiDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KanjiDataResult([])]);
        [, $err] = KanjiDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KanjiDataSpec([]), 'result' => null]);
        [, $err] = KanjiDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KanjiDataSpec(['step' => 's']),
            'result' => new KanjiDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KanjiDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KanjiDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KanjiDataSpec(['step' => 's']),
            'result' => new KanjiDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KanjiDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KanjiDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KanjiDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KanjiDataSpec([]),
            'result' => new KanjiDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KanjiDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KanjiDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new KanjiDataError('boom', 'boom');
        $utility->fetcher = function (KanjiDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KanjiDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KanjiDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KanjiDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KanjiDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KanjiDataResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KanjiDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KanjiDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KanjiDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KanjiDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KanjiDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KanjiDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KanjiDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KanjiDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KanjiDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KanjiDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KanjiDataResult(['ok' => false])]);
        $this->expectException(KanjiDataError::class);
        KanjiDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KanjiDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KanjiDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        KanjiDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KanjiDataBaseFeature();
        $b = new KanjiDataBaseFeature();
        KanjiDataFeatureAdd::call($ctx, $a);
        KanjiDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KanjiDataBaseFeature
    {
        $f = new KanjiDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        KanjiDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): KanjiDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KanjiDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KanjiDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new KanjiDataResponse(['headers' => null]),
            'result' => new KanjiDataResult([]),
        ]);
        KanjiDataResultHeaders::call($ctx);
