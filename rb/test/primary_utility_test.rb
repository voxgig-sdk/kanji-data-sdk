...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = KanjiDataOmni.make_runner(TEST_JSON_FILE, KanjiDataSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : KanjiDataOmni.errify(err)
    end
