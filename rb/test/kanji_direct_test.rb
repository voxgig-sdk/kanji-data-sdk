...
class KanjiDirectTest < Minitest::Test
  def test_direct_load_kanji
    setup = kanji_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-kanji", setup[:live] ? "live" : "unit")
    if _should_skip
...
def kanji_direct_setup(mockres)
