...
class ReadingEntity
  ...
  def make
    opts = @_entopts.dup
    ReadingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = KanjiDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Reading, Hash] the current Reading data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = KanjiDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Reading fields)
  def match_get
  ...
  #
  # @param reqmatch [ReadingLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Reading.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Reading, Hash] the loaded Reading; raises KanjiDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = KanjiDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
