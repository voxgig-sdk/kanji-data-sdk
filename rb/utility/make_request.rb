...
module KanjiDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KanjiDataResponse.new({})
    ...
    if fetch_err
      ...
      response = KanjiDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KanjiDataResponse.new(fetched)
    else
