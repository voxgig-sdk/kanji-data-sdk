...
The Ruby SDK for the KanjiData API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Kanji(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.KanjiData(nil).load(
  { "id" => "test01" }, nil
)
