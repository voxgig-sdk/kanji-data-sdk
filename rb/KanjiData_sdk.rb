...
require_relative 'KanjiData_types'
...
class KanjiDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KanjiDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KanjiDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KanjiDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KanjiDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KanjiDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KanjiDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KanjiDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = KanjiDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KanjiDataError.new(
        "graphql_error", "KanjiDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
