...
//     (\`new KanjiDataSDK(...)\` / \`KanjiDataSDK.test(...)\` become
//     \`KanjiDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"kanji":{"test01":{"id":"test01"}},"reading":{"test01":{"id":"test01"}},"word":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KanjiDataSDK(...)\` and any
// \`KanjiDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KanjiDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KanjiDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
