...
boru-driven command-line client **and** interactive REPL for the KanjiData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kanji-data-cli)
make build
...
./kanji-data-cli load 1 kanji            # {id:1} shorthand
./kanji-data-cli load '{id:1}' kanji       # explicit match map

# 5. Override the API base URL for a single call
KANJI_DATA_BASE=https://api.example.com ./kanji-data-cli load 1 kanji

# 6. No arguments -> interactive REPL
./kanji-data-cli
kanji-data> load 1 kanji
kanji-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kanji-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kanji-data-cli load 1 kanji
   ```
...
```sh
./kanji-data-cli load 1 kanji          # scalar shorthand for {id:1}
./kanji-data-cli load '{id:1}' kanji     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `kanji-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `kanji`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kanji-data-cli`. |
...
(`./kanji-data-cli <expr>`) and interactively (the REPL), and expressions compose the
