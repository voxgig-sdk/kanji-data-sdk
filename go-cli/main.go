// Command KanjiData-cli is a CLI + REPL for the KanjiData SDK,
...
const prompt = "kanji-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "kanji reading word"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KANJI_DATA_APIKEY (api key), KANJI_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KANJI_DATA_APIKEY carries the API key and
	// KANJI_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KANJI_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KANJI_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKanjiDataSDK(opts)
