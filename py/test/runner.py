...
class KanjiDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KanjiDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KanjiDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KanjiDataTestRunner.getenv("KANJI_DATA_TEST_LIVE")
        override = KanjiDataTestRunner.getenv("KANJI_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KanjiDataTestRunner.getenv(key)
        ...
        explain = KanjiDataTestRunner.getenv("KANJI_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["KANJI_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if KanjiDataTestRunner._test_control is not None:
            return KanjiDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KanjiDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KanjiDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KanjiDataTestRunner.load_test_control()
...
def load_env_local():
    KanjiDataTestRunner.load_env_local()


def env_override(m):
    return KanjiDataTestRunner.env_override(m)


def entity_data(v):
    return KanjiDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KanjiDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KanjiDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KanjiDataTestRunner.load_test_control()


def live_client_options():
    return KanjiDataTestRunner.live_client_options()


def live_delay_ms():
    return KanjiDataTestRunner.live_delay_ms()
