...
MCP server exposing the KanjiData SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `kanji-data_list` | `entity`, optional `query` map | First page of records as JSON |
| `kanji-data_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `kanji-data_list` and
`kanji-data_load` tools become available in new sessions.
