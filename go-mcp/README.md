...
[MCP](https://modelcontextprotocol.io) server exposing the KanjiData SDK as
two agent tools — `kanji-data_list` and `kanji-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kanji-data-mcp)
make build

# 2. Provide credentials via the environment
export KANJI_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kanji-data \
  -- /absolute/path/to/kanji-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kanji-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kanji-data_list: first page of records
{ "entity": "kanji" }
{ "entity": "kanji", "query": { } }

// kanji-data_load: one record by id
{ "entity": "kanji", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kanji-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kanji-data \
     -- "$PWD"/dist/*/kanji-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kanji-data_list` and `kanji-data_load` tools now appear
   in new sessions. Ask the agent to *"list kanji using kanji-data"*
   and it calls `kanji-data_list` with `{"entity":"kanji"}`.
...
```sh
export KANJI_DATA_APIKEY=sk_live_xxx            # API key
export KANJI_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `kanji-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kanji-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KANJI_DATA_APIKEY` | API key sent with every request. |
| `KANJI_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./kanji-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kanji-data_load","arguments":{"entity":"kanji","query":{"id":1}}}}'
```
