...
class FtCtrl extends KanjiDataControl
{
...
class FtClient
{
    ...
    private KanjiDataUtility $utility;

    public function __construct(KanjiDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): KanjiDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KanjiDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KanjiDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KanjiDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KanjiDataConfig::make_feature($name);
    ...
    private static function buildUrl(KanjiDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KanjiDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KanjiDataResponse($fetched);
        }
    ...
    private function fail(KanjiDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KanjiDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
