...
class KanjiDataMakeError
{
    public static function call(?KanjiDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KanjiDataContext([], null);
        }
        $op = $ctx->op ?? new KanjiDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KanjiDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KanjiDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new KanjiDataError('', $msg, $ctx);
        ...
        if ($err instanceof KanjiDataError) {
