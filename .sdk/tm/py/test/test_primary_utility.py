...
from projectname_sdk import KanjiDataSDK
from projectname_sdk.core.spec import KanjiDataSpec
from projectname_sdk.core.result import KanjiDataResult
from projectname_sdk.core.response import KanjiDataResponse
from projectname_sdk.core.operation import KanjiDataOperation
from projectname_sdk.core.error import KanjiDataError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import KanjiDataBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return KanjiDataError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = KanjiDataSDK.test(None, None)
        ...
        class TestInitFeature(KanjiDataBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(KanjiDataBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = KanjiDataSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = KanjiDataSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = KanjiDataSDK.test(None, setup_opts)
