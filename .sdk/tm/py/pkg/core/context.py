...
from projectname_sdk.core.control import KanjiDataControl
...
class KanjiDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, KanjiDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, KanjiDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, KanjiDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return KanjiDataError(code, msg, self)
